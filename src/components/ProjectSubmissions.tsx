...
import { Briefcase, UploadCloud, CheckCircle, Clock, Link as LinkIcon, FileArchive, X, Upload } from 'lucide-react';
...
export const ProjectSubmissions: React.FC<ProjectSubmissionsProps> = ({ studentId, enrollments }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedProject && !isCustomModal) return;
    if (!submissionUrl && !submissionFile) return;
    if (isCustomModal && (!customTitle.trim() || !customDescription.trim())) return;
    ...
    try {
      ...
      if (submissionFile) {
        // Read file as Data URL to mock a real upload
      ...
      const newSubmission: ProjectSubmission = {
        ...
        project_id: isCustomModal ? 'custom' : selectedProject!.id,
        student_id: studentId,
        custom_title: isCustomModal ? customTitle : undefined,
        custom_description: isCustomModal ? customDescription : undefined,
        submission_url: submissionUrl || undefined,
        submission_file: submissionFile ? submissionFile.name : undefined,
        // We can hijack submission_url to hold the file data if they only uploaded a file
        status: 'pending',
        submitted_at: new Date().toISOString()
      };
      
      if (fileDataUrl && !newSubmission.submission_url) {
        newSubmission.submission_url = fileDataUrl;
      }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex flex-col gap-1 mt-1">
                        {existingSubmission.submission_url && (
                          <a href={existingSubmission.submission_url} target="_blank" rel="noopener noreferrer" className="text-[11px] text-indigo-400 hover:text-indigo-300 flex items-center gap-1 w-fit">
                            <LinkIcon size={10} /> View Link
                          </a>
                        )}
                        {existingSubmission.submission_file && (
                          <span className="text-[11px] text-secondary/60 flex items-center gap-1">
                            <FileArchive size={10} /> {existingSubmission.submission_file}
                          </span>
                        )}
                      </div>
                    </div>
        ...
        {/* Render Custom Project Submissions */}
        {submissions.filter(s => s.project_id === 'custom').map(sub => (
          <div key={sub.id} className="bg-background-100 border border-secondary/10 rounded-xl p-8 shadow-sm flex flex-col group hover:border-indigo-500/50 transition-all">
            <div className="flex justify-between items-start mb-4">
              <div>
                <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 bg-indigo-500/10 text-indigo-400 rounded-md border border-indigo-500/20 inline-block mb-3">
                  Student Project
                </span>
                <h4 className="text-xl font-bold text-secondary line-clamp-1 group-hover:text-indigo-400 transition-colors">{sub.custom_title}</h4>
              </div>
              <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-md border ${getStatusColor(sub.status)}`}>
                {sub.status.replace('_', ' ')}
              </span>
            </div>

            <p className="text-sm text-secondary/60 mb-6 flex-1 line-clamp-3 leading-relaxed">
              {sub.custom_description}
            </p>

            <div className="mt-auto pt-6 border-t border-secondary/5 flex items-center justify-between">
              <div className="text-xs font-bold text-secondary/60">
                Max Score: <span className="text-secondary">Custom XP</span>
              </div>
              
              <div className="flex flex-col gap-2">
                <div className="text-xs font-bold flex items-center gap-2">
                  {sub.score ? (
                    <span className="text-emerald-500">Graded: {sub.score} XP</span>
                  ) : (
                    <span className="text-orange-500 flex items-center gap-1"><Clock size={14} /> Under Review</span>
                  )}
                </div>
                <div className="flex flex-col gap-1 mt-1">
                  {sub.submission_url && (
                    <a href={sub.submission_url} target="_blank" rel="noopener noreferrer" className="text-[11px] text-indigo-400 hover:text-indigo-300 flex items-center gap-1 w-fit">
                      <LinkIcon size={10} /> View Link
                    </a>
                  )}
                  {sub.submission_file && (
                    <span className="text-[11px] text-secondary/60 flex items-center gap-1">
                      <FileArchive size={10} /> {sub.submission_file}
                    </span>
                  )}
                </div>
              </div>
            ...
            {sub.feedback && (
              <div className="mt-4 p-4 bg-secondary/5 rounded-md border border-secondary/10">
                <p className="text-[10px] font-black uppercase tracking-widest text-secondary/40 mb-1">Mentor Feedback</p>
                <p className="text-xs font-medium text-secondary/80 italic">"{sub.feedback}"</p>
              </div>
            )}
          </div>
        ))}
      ...
      <div className="flex justify-center mt-8 pt-6 border-t border-secondary/10">
        <button 
          onClick={() => setIsCustomModal(true)}
          className="px-6 py-3 bg-white border border-secondary/20 hover:border-indigo-500 text-secondary font-semibold rounded-md text-sm flex items-center gap-2 transition-all shadow-sm group"
        >
          <Briefcase size={16} className="text-secondary/60 group-hover:text-indigo-500 transition-colors" />
          Add New Project
        </button>
      </div>

      {/* Submission Modal */}
      {(selectedProject || isCustomModal) && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          >
            <button 
              onClick={() => { setSelectedProject(null); setIsCustomModal(false); }} 
              className="absolute top-6 right-6 p-2 bg-secondary/5 text-secondary/40 hover:text-secondary rounded-md transition-all"
            >
              <X size={18} />
            </button>
            
            <h3 className="text-2xl font-bold text-secondary mb-2 pr-10">
              {isCustomModal ? 'Add Custom Project' : 'Submit Project'}
            </h3>
            <p className="text-sm font-medium text-secondary/60 mb-6">
              {isCustomModal ? 'Provide details for your own custom project submission.' : selectedProject?.title}
            </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      required
                      rows={3}
                      className="w-full bg-secondary/5 border border-secondary/10 rounded-md px-4 py-3.5 focus:border-indigo-500 outline-none text-secondary font-medium transition-all text-sm resize-none"
                    />
                  </div>
                </>
              )}
              <div className="space-y-4">
                ...
                <div className="relative flex py-3 items-center">
                  ...
                  <span className="flex-shrink-0 mx-4 text-xs font-bold text-secondary/40">OR</span>
                  <div className="flex-grow border-t border-secondary/10"></div>
                </div>

                <div>
                  <label className="text-[10px] font-black text-secondary/40 uppercase tracking-widest flex items-center gap-2 mb-3">
                    <Upload size={12} /> Upload Project File (ZIP / PDF)
                  </label>
                  <label className="flex items-center justify-center w-full px-4 py-4 bg-secondary/5 border-2 border-dashed border-secondary/20 hover:border-indigo-500 rounded-md cursor-pointer transition-all text-sm group">
                    <span className="text-secondary/60 group-hover:text-indigo-400 font-medium transition-colors">
                      {submissionFile ? submissionFile.name : 'Click to browse files...'}
                    </span>
                    <input
                      type="file"
                      className="hidden"
              ...
              <div className="p-4 bg-indigo-500/5 border border-indigo-500/20 rounded-md text-xs font-medium text-secondary/80 flex gap-3">
                ...
                <p>Provide a publicly accessible link OR upload your project files directly for grading.</p>
              </div>

              <button
                type="submit"
                disabled={submitting || (!submissionUrl && !submissionFile)}
