...
export const AdminCodingProblems: React.FC<AdminCodingProblemsProps> = ({
  ...
  // Boilerplate state
  const [boilerplateTab, setBoilerplateTab] = useState<'javascript' | 'python' | 'java'>('javascript');
  const [bpJs, setBpJs] = useState('');
  const [bpPy, setBpPy] = useState('');
  const [bpJava, setBpJava] = useState('');
  ...
  const handleOpenAddModal = () => {
    ...
    setDescription('');
    setConstraints('');
    setBpJs('function solution(input) {\n    // Write your code here\n    return input;\n}');
    setBpPy('def solution(input_val):\n    # Write your code here\n    return input_val');
    setBpJava('public class Solution {\n    public static String solution(String input) {\n        // Write your code here\n        return input;\n    }\n}');
              ...
              <form onSubmit={handleFormSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="border border-gray-700 rounded-xl p-4 space-y-4">
                  <div className="flex items-center justify-between border-b border-gray-700 pb-3">
                    ...
                    <div className="flex gap-1 bg-gray-900 p-0.5 rounded-lg border border-gray-750">
                  ...
                  {boilerplateTab === 'javascript' && (
                    <textarea
                      rows={5}
                      value={bpJs}
                      onChange={(e) => setBpJs(e.target.value)}
                      placeholder="// JavaScript starter boilerplate"
                      className="bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-white font-mono text-xs focus:outline-none w-full"
                    />
                  )}

                  {boilerplateTab === 'python' && (
                    <textarea
                      rows={5}
                      value={bpPy}
                      onChange={(e) => setBpPy(e.target.value)}
                      placeholder="# Python starter boilerplate"
                      className="bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-white font-mono text-xs focus:outline-none w-full"
                    />
                  )}
                  ...
                  {boilerplateTab === 'java' && (
                    <textarea
                      rows={5}
                      value={bpJava}
                      onChange={(e) => setBpJava(e.target.value)}
                      placeholder="// Java starter boilerplate"
                      className="bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-white font-mono text-xs focus:outline-none w-full"
                    />
                  )}
                </div>
