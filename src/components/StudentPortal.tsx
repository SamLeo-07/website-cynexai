...
const StudentPortal = () => {
  ...
  return (
    <div className="h-screen overflow-hidden bg-background text-secondary flex flex-col lg:flex-row selection:bg-indigo-500/20">
      {/* Sidebar Navigation */}
      <aside className="w-80 bg-background-100 border-r border-secondary/10 hidden lg:flex flex-col sticky top-0 h-screen pt-24 pb-8">
        <div className="px-6 mb-12">
          <div className="p-6 bg-secondary/5 rounded-md border border-secondary/10 relative overflow-hidden group">
            <div className="absolute inset-0 bg-indigo-500/5 opacity-0 group-hover:opacity-100 transition-opacity" />
            <h2 className="font-bold text-secondary mb-1 relative z-10">{studentName}</h2>
            <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest relative z-10">Student Identity Verified</p>
          </div>
        </div>
        ...
        <nav className="flex-1 px-4 space-y-2 overflow-y-auto no-scrollbar">
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-4 px-6 py-3 rounded-md font-bold transition-all ${
                activeTab === item.id 
        ...
        <div className="px-4 mt-auto">
          <button
            ...
            className="w-full flex items-center gap-4 px-6 py-3 rounded-md font-bold text-red-500 hover:bg-red-500/10 transition-all"
          >
      ...
      {/* Main Content Area */}
      <main className="flex-1 overflow-y-auto overflow-x-hidden pt-28 lg:pt-24 pb-24 lg:pb-12 px-3 sm:px-6 lg:px-12 relative">
        {/* Mobile Top Bar (only visible on mobile) */}
        <div className="lg:hidden fixed top-14 left-0 right-0 z-40 bg-background-100/95 backdrop-blur-xl border-b border-secondary/10 px-4 py-2 flex items-center justify-between">
          <div>
            <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Welcome</p>
            <h3 className="font-bold text-secondary text-sm truncate max-w-[160px]">{studentName}</h3>
          </div>
          <button
            onClick={handleLogout}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[10px] font-black text-red-500 bg-red-500/10 hover:bg-red-500/20 transition-all uppercase tracking-wider"
          >
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          ...
          <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-background-100/95 backdrop-blur-xl border-t border-secondary/10 pb-safe rounded-t-lg">
              ...
              {navItems.map((item) => (
                <button
                  ...
                  className={`flex flex-col items-center justify-center gap-0.5 flex-shrink-0 px-3 h-full transition-all ${
                    activeTab === item.id ? 'text-indigo-400' : 'text-secondary/40'
                  }`}
                  title={item.label}
                >
                  <div className={`p-1.5 rounded-md transition-all ${activeTab === item.id ? 'bg-indigo-500/15' : ''}`}>
        ...
        {isCurriculumModalOpen && selectedCurriculumCourse && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative bg-[#0f172a] border border-slate-800 rounded-xl p-6 md:p-8 w-full max-w-3xl max-h-[85vh] overflow-y-auto shadow-2xl text-white"
            >
              <button 
                onClick={() => setIsCurriculumModalOpen(false)} 
              ...
              <div className="mb-6">
                <span className="px-3 py-1 bg-indigo-500/10 text-indigo-400 text-[10px] font-black rounded-lg uppercase tracking-widest border border-indigo-500/20">
                  {batchesList.find(b => b.course_id === selectedCurriculumCourse.id && b.id === studentBatchId)?.name || 'General Section'}
                </span>
                <h3 className="text-2xl font-bold text-white mt-3 mb-2">{selectedCurriculumCourse.title}</h3>
                <p className="text-xs text-slate-400 font-medium font-sans">Day-by-day schedules, assignments, and allocated resources</p>
              </div>
              ...
              {(() => {
                ...
                if (!hasSchedule && !hasTests && !hasTools && !hasTips) {
                  return (
                    <div className="text-center py-12 bg-slate-950/40 rounded-[1.5rem] border border-slate-900">
                      <BookOpen className="w-12 h-12 text-slate-700 mx-auto mb-3" />
                      <p className="text-sm text-slate-400 italic">No curriculum details allocated for this course yet.</p>
                    </div>
                  );
                }
                ...
                return (
                  <div className="space-y-6">
                    ...
                    {hasSchedule && (
                      <div className="space-y-3">
                        <h4 className="text-sm font-semibold text-slate-300 uppercase tracking-wider flex items-center gap-2">
                          <span className="w-1.5 h-1.5 rounded-full bg-indigo-400" /> Daily Schedule
                        </h4>
                        <div className="space-y-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                          {parsedCurriculum.days.map((day: any, i: number) => (
                            <div key={i} className="bg-[#0a0a0a] border border-slate-800 rounded-xl p-4 flex flex-col sm:flex-row justify-between gap-3 sm:items-center">
                              <div>
                                <span className="text-[10px] font-black text-indigo-400 uppercase tracking-wider">
                                  Day {day.dayNumber} {day.date && <span className="ml-2 text-slate-500 font-normal">({day.date})</span>}
                                </span>
                                <h5 className="text-sm font-semibold text-slate-200 mt-0.5">{day.concept || 'TBD'}</h5>
                              </div>
                              <div className="flex flex-wrap gap-4 text-xs font-medium">
                                {day.material && (
                                  <div className="text-slate-400">
                                    <span className="text-[10px] text-slate-600 block uppercase font-bold">Material</span>
                                    {day.material.startsWith('http') ? (
                                      <a href={day.material} target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:underline">Link &rarr;</a>
                                    ) : (
                                ...
                                {day.assignment && (
                                  <div className="text-slate-400">
                                    <span className="text-[10px] text-slate-600 block uppercase font-bold">Assignment</span>
                                    {(() => {
                                      const linkedTest = mockTests.find(t => t.id === day.assignment);
                                      if (linkedTest) {
                                        return (
                                          <div className="flex items-center gap-2 mt-1">
                                            <span className="text-indigo-300 font-medium truncate max-w-[150px]">{linkedTest.title}</span>
                                            <button
                                              onClick={() => {
                    ...
                    {hasTests && (
                      <div className="space-y-2">
                        <h4 className="text-sm font-semibold text-slate-300 uppercase tracking-wider flex items-center gap-2">
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" /> Weekly Tests & Assessments
                        </h4>
                        <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {parsedCurriculum.weeklyTests.map((testIdOrText: string, i: number) => {
                            ...
                            if (linkedTest) {
                              return (
                                <li key={i} className="bg-indigo-600/10 border border-indigo-500/20 rounded-xl p-3 text-xs text-slate-200 flex items-center justify-between gap-2 shadow-sm">
                            ...
                            return (
                              <li key={i} className="bg-slate-950/20 border border-slate-900/50 rounded-xl p-3 text-xs text-slate-300 flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      ...
                      {hasTools && (
                        <div className="space-y-2">
                          <h4 className="text-sm font-semibold text-slate-300 uppercase tracking-wider flex items-center gap-2">
                            <span className="w-1.5 h-1.5 rounded-full bg-indigo-400" /> Required Tools & Software
                          </h4>
                          <div className="bg-[#0a0a0a] border border-slate-800 rounded-xl p-4 space-y-1.5">
                      ...
                      {hasTips && (
                        <div className="space-y-2">
                          <h4 className="text-sm font-semibold text-slate-300 uppercase tracking-wider flex items-center gap-2">
                            <span className="w-1.5 h-1.5 rounded-full bg-amber-400" /> Professional Tips & Hacks
                          </h4>
                          <div className="bg-[#0a0a0a] border border-slate-800 rounded-xl p-4 space-y-1.5">
