...
// ─── Constants & Utils ────────────────────────────────────────────────────────
const GRID_SIZE = 25;     // Number of points along X and Z axes (reduced from 40 for speed)
const SPACING = 2.4;      // Distance between points (scaled from 1.5 to maintain bounds size)
...
const AmbientGlows = () => {
    ...
    const glows = useMemo(() => [
        ...
        { color: isDarkMode ? '#1c2541' : '#ffffff', pos: [20, -10, -40], scale: 50, speed: -0.03 },
...
const KnowledgeGrid = () => {
    ...
    useFrame((state) => {
        ...
        for (let i = 0; i < PARTICLE_COUNT; i++) {
            ...
            // Interactive "Ripple" from Mouse (Optimized: skip Math.sqrt unless within bounds)
            const dx = px - mouseX;
            const dz = pz - mouseZ;
        ...
        // Sync lines to new points (Optimized: use native browser Array.set instead of custom loop copy)
        if (linesRef.current) {
...
// ─── Smooth Camera Controller (Optimized: event-based scroll layout checks) ───
...
const ThreeBackground = () => {
    ...
    return (
        <div className={`fixed inset-0 z-[-1] pointer-events-none transition-colors duration-1000 three-background ${isDarkMode ? 'dark' : ''}`}>
            ...
            <div className={`absolute inset-0 transition-colors duration-1000 ${isDarkMode ? 'bg-[#0b132b]' : 'bg-[#ffffff]'}`} />
            ...
            >
                <CameraController />
                <fog attach="fog" args={[isDarkMode ? '#0b132b' : '#ffffff', 10, 50]} />
