...
export const AdminAttendance: React.FC<AdminAttendanceProps> = ({
  ...
  const handleCreateSession = async (e: React.FormEvent) => {
    ...
    // Generate random 6 digit pin (kept for backward compatibility or edge cases)
    const pin = Math.floor(100000 + Math.random() * 900000).toString();
    const meetingLink = createFormData.meeting_link || `https://meet.cynexai.in/${crypto.randomUUID()}`;
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6 shrink-0 bg-gray-900 p-6 rounded-2xl border border-gray-700">
            <div className="text-center space-y-3">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Meeting Controls</span>
              <a 
                href={activeSessionForPin.meeting_link} 
                target="_blank" 
                rel="noreferrer"
                className="block px-6 py-3 bg-[#41c8df] text-black font-black text-sm uppercase tracking-wider rounded-xl hover:bg-[#32adbc] transition-all"
              >
                Start Meeting
              </a>
              <div className="text-[10px] text-gray-500 font-medium">Students will join automatically</div>
            </div>
          </div>
        </motion.div>
      )}
            ...
            >
              ...
              <form onSubmit={handleCreateSession} className="p-6 space-y-6">
                ...
                <div className="space-y-2">
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Meeting Link (Optional)</label>
                  <div className="flex gap-3">
                    <input
                      type="url"
                      value={createFormData.meeting_link}
                      onChange={(e) => setCreateFormData({ ...createFormData, meeting_link: e.target.value })}
                      placeholder="https://zoom.us/j/123... or leave empty to auto-generate"
                      className="flex-1 bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-white focus:border-[#41c8df] focus:outline-none text-sm"
                    />
                    <button 
                      type="button"
                      onClick={() => setCreateFormData(prev => ({ ...prev, meeting_link: `https://meet.cynexai.in/${crypto.randomUUID().slice(0, 8)}` }))}
                      className="px-4 py-3 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-xl font-bold text-xs whitespace-nowrap hover:bg-indigo-500/20 transition-all"
                    >
                      Auto-generate
                    </button>
                  </div>
                ...
                <button
                  type="submit"
                  className="w-full py-3.5 bg-[#41c8df] hover:bg-[#2bb5cc] text-black font-bold rounded-xl transition-all shadow-lg shadow-[#41c8df]/15 text-sm uppercase tracking-wider"
                >
