...
export const StudentAttendance: React.FC<StudentAttendanceProps> = ({
  ...
  const handleJoinMeeting = async (session: AttendanceSession & { attended?: boolean }) => {
    if (!session) return;
    
    // Open meeting link immediately so user isn't blocked by slow DB
    ...
    try {
      ...
      setMessage({ type: 'success', text: `Attendance marked automatically for ${session.topic}!` });
      
      // Refresh statistics and list
      await fetchAttendanceData();
    } catch (e) {
      setMessage({ type: 'error', text: 'An error occurred while marking attendance in the background.' });
      console.error(e);
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Right Panel: Mark Attendance Meeting Flow */}
        <div className="space-y-8">
          <div className="bg-background-100 border border-secondary/10 rounded-xl p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="font-bold text-lg text-secondary">Join Live Class</h3>
                <p className="text-xs text-secondary/60 font-medium">Select course to see active sessions.</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              {(() => {
                ...
                const activeSession = sessions.find(s => s.course_id === selectedCourseId && s.session_date === today);
                
                if (activeSession) {
                  return (
                    <div className="border border-indigo-200 bg-indigo-50/30 rounded-xl p-5 space-y-4">
                      <div className="space-y-1 text-center">
                        <span className="px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded text-[9px] font-black uppercase tracking-widest mb-2 inline-block">Active Now</span>
                        <h4 className="font-bold text-indigo-950 text-lg">{activeSession.topic}</h4>
                        {activeSession.session_time && <p className="text-xs text-indigo-800 font-medium">{activeSession.session_time}</p>}
                      </div>
                      
                      <button
                        onClick={() => handleJoinMeeting(activeSession)}
                        disabled={submitting}
                        className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-md transition-all disabled:opacity-50 shadow-md shadow-indigo-900/10 text-sm flex items-center justify-center gap-2"
                      >
                        {submitting ? (
                          <div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin" />
                        ) : (
                          <>
                            {activeSession.attended ? 'Re-join Meeting' : 'Join Meeting & Check-in'}
                          </>
                        )}
                      </button>
                    </div>
                ...
                return (
                  <div className="text-center py-8 bg-secondary/5 rounded-xl border border-secondary/10 border-dashed">
                    <Calendar className="w-8 h-8 text-secondary/20 mx-auto mb-2" />
                    <p className="text-sm font-bold text-secondary/40">No active classes today.</p>
                  </div>
                );
              })()}
            </div>
          ...
          <div className="bg-indigo-50 border border-indigo-100 text-indigo-950 rounded-xl p-6 relative overflow-hidden shadow-sm">
            <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/5 rounded-full blur-3xl" />
            <h4 className="font-bold text-base mb-3 text-indigo-700">How to check-in?</h4>
            <ul className="space-y-3 text-xs text-indigo-900/85 font-medium">
              <li className="flex gap-2">
                <span className="text-indigo-600 font-black">1.</span>
                When your instructor starts a class, an "Active Now" card will appear above.
              </li>
              <li className="flex gap-2">
                <span className="text-indigo-600 font-black">2.</span>
                Click the "Join Meeting & Check-in" button.
              </li>
              <li className="flex gap-2">
                <span className="text-indigo-600 font-black">3.</span>
                Your presence is logged automatically in the background as you are redirected to the live meeting.
              </li>
            </ul>
          </div>
