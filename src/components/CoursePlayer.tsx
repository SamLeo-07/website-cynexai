...
import { 
  ...
  getAttendanceSessions, createAttendanceSession, markAttendance, AttendanceSession, AttendanceRecord
} from '../lib/turso';
...
const CoursePlayer: React.FC<CoursePlayerProps> = ({ 
  ...
  useEffect(() => {
    // Automatically mark attendance for Advanced courses when they open the player
    const markAutoAttendance = async () => {
      if (course.level === 'Advanced') {
        const today = new Date().toISOString().split('T')[0];
        const sessionId = `auto_sess_${course.id}_${today}`;
        
        try {
          const sessions = await getAttendanceSessions(course.id);
          ...
          const record: AttendanceRecord = {
            id: `att_rec_${enrollment.student_id}_${sessionId}`,
    ...
    markAutoAttendance();
  }, [course, enrollment.student_id, studentName]);
