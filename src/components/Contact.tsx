    ...
    <section id="contact" className="py-20 relative bg-transparent text-secondary overflow-hidden relative z-10">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <motion.p variants={itemVariants} className="text-lg text-slate-600 dark:text-gray-300">
                ...
                {contactInfo.map((item, i) => {
                  ...
                  return (
                    <motion.a
                      ...
                      className="flex items-center space-x-4 p-4 bg-background-100/40 backdrop-blur-xl dark:bg-gray-900/40 rounded-xl border border-secondary/10 hover:border-[#41c8df] hover:shadow-[0_0_20px_rgba(65,200,223,0.2)] transition-all duration-300 contact-preserve-3d"
                    >
                      ...
                      <div>
                        <p className="text-sm text-slate-500 dark:text-gray-400 font-medium">{item.label}</p>
                ...
                {stats.map((stat, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="p-6 bg-background-100/60 backdrop-blur-xl dark:bg-gray-900/60 rounded-xl border border-secondary/10 text-center hover:border-[#41c8df]/60 hover:shadow-[0_10px_30px_rgba(65,200,223,0.2)] transition-all duration-300 cursor-default contact-stat-card shadow-sm"
                    >
                      ...
                      <div className="text-sm text-slate-500 dark:text-gray-400 font-medium">{stat.label}</div>
                    </motion.div>
                  );
                })}
              </div>
            ...
            <form className="space-y-5" onSubmit={handleSubmit}>
              {[
                ...
                <motion.div key={field.id} variants={formFieldVariants} className={`contact-field-delay-${i}`}>
                  <label htmlFor={field.id} className="block text-sm text-slate-500 dark:text-gray-400 mb-2">{field.label}</label>
                  <input
                    type={field.type}
                    id={field.id}
                    name={field.id}
              ...
              <motion.div variants={formFieldVariants}>
                <label htmlFor="courseInterest" className="block text-sm text-slate-500 dark:text-gray-400 mb-2">Course Interest</label>
                <select
                  id="courseInterest"
                  name="courseInterest"
              ...
              <motion.div variants={formFieldVariants}>
                <label htmlFor="message" className="block text-sm text-slate-500 dark:text-gray-400 mb-2">Message</label>
                <textarea
                  id="message"
                  name="message"
                  rows={4}
