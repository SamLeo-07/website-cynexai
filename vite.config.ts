...
export default defineConfig({
  ...
  // Dev-server proxy: forwards /attendance/* to the Next.js attendance app.
  // This mirrors the _redirects file behaviour used in production.
  server: {
    proxy: {
      '/attendance': {
        target: 'http://localhost:3001',
        changeOrigin: true,
        // Don't rewrite the path — Next.js expects /attendance as its basePath
      },
    },
  },
  build: {
